	...
	public static List<Movie> nsearchContent(SearchPageBean pagebean) {
		...
		try {
			...
			String sql_str = "select movies.id, title, year, dirctor, banner_url from stars, movies, stars_in_movies "
					...
					+ "and movies.title like '%" + pagebean.getTitle() + "%' and dirctor like '%"
					+ pagebean.getDirector() + "%' and year >= " + pagebean.getSyear() + " and year <="
					+ pagebean.getEyear() + " " + "and first_name like '%" + pagebean.getFname()
					+ "%' and last_name like '%" + pagebean.getLname() + "%' group by movies.id limit " + start + ","
