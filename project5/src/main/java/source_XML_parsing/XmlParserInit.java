...
public class XmlParserInit extends DefaultHandler {
	...
	public void characters(char[] c, int s, int l) throws SAXException {
		super.characters(c, s, l);
		this.value = new String(c, s, l);
		if (value == null) {
			this.value = "";
		}
	}
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		...
		if (qName.equals("actor")){
			...
			star.setL_Name(value);
		
		}else if (qName.equals("firstname")){
			
			star.setF_Name(value);
		...
		// for mains243.xml
			...
			if (names.length<2){
				star.setF_Name(names[0]);
				star.setL_Name("");
			}else{
				star.setF_Name(names[0]);
				star.setL_Name(names[names.length-1]);
			}
