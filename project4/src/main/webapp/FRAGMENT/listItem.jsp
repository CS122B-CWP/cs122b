<div class="col-md-6 FX-col">
	<li class="list-group-item FX-listItem pre">
		<div class="FX-innerItem FX-listLeft">
			<a href="#"> <img
				src="http://ia.imdb.com/media/imdb/01/I/95/71/38m.jpg"
				alt="Generic placeholder image" style="height: 200px; width: 100%;">
				<p></p>
				<p>id - Title</p>
			</a>
			<p>Director</p>
		</div>
		<div class="FX-innerItem FX-listRight">
			Year:
			<hr>
			Genres: <a href="">absa</a>
			<hr>
			Stars: <a href="">blabla</a>
		</div> <br>
		<div class="FX-innerItem FX-listBuy">
			<%
				//pass movie id by url
			%>
			<form action="" method="POST" role="form">
				<button type="button" class="btn btn-success btn-sm">Buy</button>
				<div class="FX-QtyContainer">
					<div class="input-group">
						<span class="input-group-addon" id="basic-addon1">Qty</span> <input
							type="text" class="form-control input-sm"
							aria-describedby="basic-addon1">
					</div>
				</div>
			</form>
		</div>
	</li>
</div>